...
let joinRoomInit = async () => {
  let response = await fetch(
    `https://mumble2.herokuapp.com/get_rtm_token?uid=${uid}`
  )
...
let getRoomsData = async () => {
  ...
  for (let i = 0; rooms.length > i; i++) {
    ...
    let {
      room_name,
      host
    } = await rtmClient.getChannelAttributesByKeys(roomName, [
      'room_name',
      'host'
    ])
  ...
  await rtmClient.addOrUpdateChannelAttributes(roomId, {
    rooms: JSON.stringify(newRoomsList)
  })

  channel.sendMessage({
    text: JSON.stringify({ type: 'rooms_update', roomsData: roomsData })
  })
...
let buildRooms = async roomsData => {
  ...
  document.getElementById('rooms__count').innerHTML = `Rooms: ${count}`
  ...
  for (key in roomsData) {
    ...
    let room = `<div id="room-${key}" class="room__item">
                        <div class="room__content">
                            ...
                            <h4 class="room__title">${roomsData[key][
                              'room_name'
                            ].value}</h4>
                            <div class="room__box">
                                <div class="room__author">
                                    <strong class="message__author">${roomsData[
                                      key
                                    ]['host'].value}</strong>
                                </div>
                                <a class="room__action" target="_blank" href="lobbyLive?room=${key}">Join Now</a>
                            </div>
                        </div>
                    </div>`

    // `<div id="room-${key}">
    //                 <h4><a href="lobbyLive.html?room=${key}">${roomsData[key]['room_name'].value}</a></h4>
